...
int kmain(unsigned int multiboot_info_addr)
{
    ...
    /* Debug: mostra os endereços do módulo para verificar se estão corretos */
    klog_write("Debug: mods_addr = ");
    klog_write_hex(mbi->mods_addr);
    klog_write("\n");
    klog_write("Debug: mod_start = ");
    klog_write_hex(mod->mod_start);
    klog_write("  mod_end = ");
    klog_write_hex(mod->mod_end);
    klog_write("\n");
